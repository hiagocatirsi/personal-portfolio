...
(() => {
  ...
  class ImageSlot extends HTMLElement {
    ...
    async _ingest(file) {
      ...
      try {
        // Floor of 640 (→ up to 1200 after the 2× retina cap) avoids blurry
        // uploads on slots that render small only transiently — a collapsed
        // carousel card, a not-yet-centered item — but will later show large.
        // Never upscales past the source photo's own resolution.
        const w = Math.max(this.clientWidth || this.offsetWidth || MAX_DIM, 640);
